...
/**
 * Client-only entry point (specification 10): the client config, the terminal and shop screens, the quota board
 * renderer, the HUD layer, the toggle key and the clientbound payload handlers. Never loaded on a dedicated server, so
 * nothing outside this class and the {@code client} package may touch {@code net.minecraft.client}.
 */
...
public final class ConsortiumCoreClient {
    public ConsortiumCoreClient(IEventBus modBus, ModContainer container) {
        ...
        ConsortiumNetwork.setClientHandlers(ClientSetup::handleBalance, ClientSetup::handleQuote, ClientSetup::handleBoard,
                ClientSetup::handleShopCatalog, ClientSetup::handleShopResult);
        ...
        ConsortiumCore.LOGGER.info("Consortium Core client wiring ready (terminal and shop screens, quota board renderer, credits HUD, toggle key)");
    }
}
